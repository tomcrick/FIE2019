...
\\(Abstract 20 January, Paper 27th)}
...
\begin{abstract}
There are many facets to CyberSecurity education: we address the questions ``what should the generalist computer scientist know'' and ``how well is that being taught''.  (Cyber)Security (if thought about at all) has hisorically been left to specialists, sometimes called ``Information Assurance'', which worked, even if not well, in a waterfall model.  The Agile and DevOps trends have changed devleopment, but not themselves the view of security. Over the last few years, there has been a recogition of the need for change, exemplified by ``DevSecOps''. But if security is not to be left to the experts, then the generalist must know about it.

We consider particularly the UK perspective, where the British Computer Society accredits computing degrees, and (since \IntroYear) insists on a certain CyberSecurity element in all such.
\end{abstract}
